      ...
      <aside className="w-56 bg-white border-r border-gray-200 flex flex-col shrink-0">
        <div className="p-5 border-b border-gray-100">
          <Link href="/admin" className="flex items-center gap-2">
            ...
            <span className="font-bold text-gray-800">BUpack 后台</span>
          </Link>
        </div>

        <nav className="flex-1 p-3 space-y-1">
          <NavItem href="/admin" icon={DashboardIcon} label="仪表盘" />
          <NavItem href="/admin/blogs" icon={BlogIcon} label="博客管理" />
          <NavItem href="/admin/inquiries" icon={InquiryIcon} label="询盘管理" />
          <NavItem href="/admin/products" icon={ProductIcon} label="产品管理" />
          <NavItem href="/admin/seo" icon={SeoIcon} label="SEO概览" />
        </nav>

        <div className="p-4 border-t border-gray-100">
          <div className="text-xs text-gray-400 mb-2">账户</div>
          <LogoutButton />
        </div>
      </aside>
